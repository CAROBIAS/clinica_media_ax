...
async function getPacienteIdByUsuarioId(usuarioId) {
  const [rows] = await pool.query('SELECT id_paciente FROM pacientes WHERE id_usuario = ?', [usuarioId]);
  return rows.length > 0 ? rows[0].id_paciente : null;
}

async function getMedicoIdByUsuarioId(usuarioId) {
  const [rows] = await pool.query('SELECT id_medico FROM medicos WHERE id_usuario = ?', [usuarioId]);
...
export const getTurnos = async (req, res, next) => {
  try {
    ...
    let query = `
      SELECT t.*, 
             m.id_medico, u_med.apellido as medico_apellido, u_med.nombres as medico_nombres,
             p.id_paciente, u_pac.apellido as paciente_apellido, u_pac.nombres as paciente_nombres,
             os.nombre as obra_social_nombre, os.porcentaje_descuento
...
export const getTurnoById = async (req, res, next) => {
  try {
    ...
    `, [id]);
    if (!rows.length) return res.status(404).json({ ok: false, message: "Turno no encontrado" });
...
export const createTurno = async (req, res, next) => {
  try {
    ...
    if (!id_medico || !id_paciente || !fecha_hora) {
      return res.status(400).json({ ok: false, message: "Faltan campos obligatorios: id_medico, id_paciente, fecha_hora" });
    }
    ...
    if (!id_obra_social) {
      ...
      if (particular.length === 0) {
        return res.status(500).json({ ok: false, message: "No hay una obra social marcada como 'particular'. Cree una con es_particular=1." });
      }
    ...
    if (!medicos.length) return res.status(400).json({ ok: false, message: "Médico inválido o inactivo" });
    const valorConsulta = parseFloat(medicos[0].valor_consulta);

    const [pacientes] = await pool.query(
      "SELECT p.id_paciente FROM pacientes p JOIN usuarios u ON p.id_usuario = u.id_usuario WHERE p.id_paciente = ? AND u.activo = 1",
      [id_paciente]
    );
    if (!pacientes.length) return res.status(400).json({ ok: false, message: "Paciente inválido o inactivo" });

    const [obras] = await pool.query(
      "SELECT porcentaje_descuento, es_particular FROM obras_sociales WHERE id_obra_social = ? AND activo = 1",
      [id_obra_social]
    );
    if (!obras.length) return res.status(400).json({ ok: false, message: "Obra social inválida o inactiva" });
    
    let valorTotalCalculado = valorConsulta;
    const obra = obras[0];
    if (obra.es_particular === 0) {
      const descuento = parseFloat(obra.porcentaje_descuento);
    ...
    if (ocupado.length) return res.status(400).json({ ok: false, message: "Horario no disponible" });
...
export const updateTurno = async (req, res, next) => {
  try {
    ...
    if (!fecha_hora) return res.status(400).json({ ok: false, message: "fecha_hora es requerida" });
    await pool.query("UPDATE turnos_reservas SET fecha_hora = ? WHERE id_turno_reserva = ? AND activo = 1", [fecha_hora, id]);
...
export const marcarAtendido = async (req, res, next) => {
  try {
    const { id } = req.params;
    const userId = req.user.id; // id_usuario del token

    // Obtener id_medico del usuario logueado
    ...
    if (turno.length === 0) {
      return res.status(404).json({ ok: false, message: "Turno no encontrado o no pertenece a este médico" });
    }
